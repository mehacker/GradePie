        ...
        if pieData != nil
        {
            for set in pieData!.dataSets as! [IPieChartDataSet]
            {
                if set.isVisible && set.entryCount > 0
                {
                    ...
                    drawSecondLayer(context: context, dataSet: set)
                    drawMaskingLayer(context: context, dataSet: set)
                }
            }
        }
    }
...
//                  context.strokePath()
            ...
            for j in 0 ..< dataSet.entryCount
            {
                ...
                if drawXInside || drawYInside
                {
                    ...
                    if drawXInside && drawYInside
                    {
                        ChartUtils.drawText(
                            context: context,
                            //Text changed to include percentage
                            text: valueText + "%",
    ...
    open func drawMaskingLayer (context: CGContext, dataSet: IPieChartDataSet) {
        guard
            let chart = chart,
            let animator = animator
            else {return}
        ...
        let entryCount = dataSet.entryCount
        var drawAngles = chart.drawAngles
        var drawAngles2 = chart.secondaryAngles
        let center = chart.centerCircleBox
        let radius = chart.radius
        let drawInnerArc = chart.drawHoleEnabled && !chart.drawSlicesUnderHoleEnabled
        let userInnerRadius = drawInnerArc ? radius * chart.holeRadiusPercent : 0.0
...
//                    context.setFillColor(UIColor.red.cgColor)
                    ...
                    let startAngleOuter = rotationAngle + (angle + sliceSpaceAngleOuter / 2.0) * CGFloat(phaseY)
                    var sweepAngleOuter = (sliceAngle - sliceSpaceAngleOuter) * CGFloat(phaseY)
                    var sweepAngleOuter2 = (sliceAngle2 - sliceSpaceAngleOuter) * CGFloat(phaseY)
                    if sweepAngleOuter < 0.0
                    {
                        sweepAngleOuter = 0.0
                    }
                    ...
                    path.addRelativeArc(center: center, radius: radius, startAngle: startAngleOuter * ChartUtils.Math.FDEG2RAD, delta: sweepAngleOuter2 * ChartUtils.Math.FDEG2RAD)
                        ...
                        (innerRadius > 0.0 || accountForSliceSpacing)
                    {
                        ...
                        let sliceSpaceAngleInner = visibleAngleCount == 1 || innerRadius == 0.0 ?
                            ...
                            sliceSpace / (ChartUtils.Math.FDEG2RAD * innerRadius)
                        let startAngleInner = rotationAngle + (angle + sliceSpaceAngleInner / 2.0) * CGFloat(phaseY)
                        var sweepAngleInner = (sliceAngle2 - sliceSpaceAngleInner) * CGFloat(phaseY)
                        if sweepAngleInner < 0.0
                        {
                            sweepAngleInner = 0.0
                        }
    ...
    open func drawSecondLayer (context: CGContext, dataSet: IPieChartDataSet) {
        guard
            let chart = chart,
            let animator = animator
            else {return }
